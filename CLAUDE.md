...
- `make fuzz CC=clang` builds `test/fuzz` (libFuzzer, ASan+UBSan): what the other side sends, the agent's messages and SFTP replies. The first byte of an input picks which. `test/fuzz -max_total_time=60`. It is **not** built by `make test`, so it rots quietly: build it after changing `agent_read` or the SFTP calls it uses (CI's fuzz job is what catches it otherwise).
- CI:
